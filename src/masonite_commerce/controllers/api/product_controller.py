...
class ProductController(Controller):
    ...
    def index(self):
        ...
        comment_query = JoinClause("commerce_comments as comments").on("comments.product_id", "=", "commerce_products.id")
        meta_query = JoinClause("commerce_metas as metas").on("metas.product_id", "=", "commerce_products.id")
        
        products = CommerceProduct.select_raw("""
            commerce_products.*,
        ...
        """).join(comment_query).join(meta_query).group_by("comments.product_id, metas.id").paginate(per_page, page)
